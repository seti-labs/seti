...
export const usersApi = {
  ...
  getPreferences: async (address: string) => {
    // Fetch backend preferences and map to frontend shape
    const resp = await apiFetch<{ preferences: any }>(`/users/${address}/preferences`);
    const backend = resp.preferences || {};

    // Map backend theme -> frontend theme_preference
    const theme_preference: 'light' | 'dark' | 'system' =
      backend.theme === 'light' || backend.theme === 'dark' ? backend.theme : 'system';

    // Backend only has a single notifications toggle; map to all three frontend toggles
    const notificationsEnabled = typeof backend.notifications === 'boolean' ? backend.notifications : true;

    const mapped = {
      // keep optional profile fields if backend includes them
      username: backend.username,
      avatar_url: backend.avatar_url,
      bio: backend.bio,
      notification_settings: {
        marketUpdates: notificationsEnabled,
        positionAlerts: notificationsEnabled,
        marketResolution: notificationsEnabled,
      },
      theme_preference,
      first_seen: backend.first_seen,
      last_active: backend.last_active,
    };
  ...
  updatePreferences: async (address: string, data: {
    ...
    if (data.theme_preference !== undefined) {
      // frontend 'system' maps to backend 'auto'
      payload.theme = data.theme_preference === 'system' ? 'auto' : data.theme_preference;
    }

    if (data.notification_settings !== undefined) {
      const ns = data.notification_settings as {
        marketUpdates?: boolean;
        positionAlerts?: boolean;
        marketResolution?: boolean;
      };
      // Backend has a single notifications boolean; derive as OR of the three toggles
      const anyEnabled = [ns.marketUpdates, ns.positionAlerts, ns.marketResolution]
        .filter(v => typeof v === 'boolean')
        .some(v => v as boolean);
      // If none provided, keep undefined to avoid overwriting unintentionally
      if (
        ns.marketUpdates !== undefined ||
        ns.positionAlerts !== undefined ||
        ns.marketResolution !== undefined
      ) {
        payload.notifications = anyEnabled;
      }
    }
